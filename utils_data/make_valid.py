...
parser.add_argument("--save_dir", type=str, default='testset/DIV2K_valid_HR/', help='the save path of the training dataset.')
...
parser.add_argument("--epoch", type=int, default=30, help='decide how many epochs to create for the dataset.')
...
with torch.no_grad():
    for epoch in range(epochs):
        gt_path = os.path.join(root_path, 'HR')
        ...
        for num_batch, batch in enumerate(train_dataloader):
            ...
            for i in range(batch_size):
                ...
                lr_save_path =  os.path.join(lr_path, os.path.basename(batch["gt_path"][i]).replace(".png", "_"+ str(5)+".png"))
                ...
                #with open(args.save_dir + "abnormal.txt", 'a') as f:
                #    f.write(os.path.basename(lr_save_path) + " " + os.path.basename(batch["gt_path"][i]))  
