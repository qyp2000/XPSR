...
parser.add_argument("--save_dir", type=str, default='trainset', help='the save path of the training dataset.')
parser.add_argument("--start_gpu", type=int, default=60, help='if you have 5 GPUs, you can set it to 1/2/3/4/5 on five gpus for parallel processing., which will save your time. ') 
parser.add_argument("--end_gpu", type=int, default=61, help='if you have 5 GPUs, you can set it to 1/2/3/4/5 on five gpus for parallel processing., which will save your time. ')   
...
class LocalImageDataset(data.Dataset):
    def __init__(self, 
        ...
        for index in range(start, end):
            for folder in os.listdir(f'{pngtxt_dir}/dataset_0/HR'):
                self.img_paths.extend(sorted(glob.glob(f'{pngtxt_dir}/dataset_{str(index)}/HR/{folder}/*.png')))
                os.makedirs(f'{pngtxt_dir}/dataset_{str(index)}/highlevel_prompt/{folder}', exist_ok=True)
                os.makedirs(f'{pngtxt_dir}/dataset_{str(index)}/highlevel_prompt_GT/{folder}', exist_ok=True)
    ...
    def __getitem__(self, index):
        ...
        image_llava_input = [hr_image_read, lr_image_read]
        prompt_llava_input = [highlevel_prompt, highlevel_prompt]
        path = [highlevel_prompt_gt_path, highlevel_prompt_path]
...
train_dataset_loader = accelerator.prepare(
...
with torch.no_grad():
    for i, example in enumerate(tqdm(train_dataset_loader, disable=not accelerator.is_local_main_process)):
        ...
        if os.path.exists(paths[0][0]) and  os.path.exists(paths[1][0]):
            continue
