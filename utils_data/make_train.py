...
parser.add_argument("--gt_path", nargs='+', default='gt_path/', help='the path of high-resolution images')
parser.add_argument("--save_dir", type=str, default='trainset/dataset', help='the save path of the training dataset.')
...
folders = os.listdir(args.gt_path)
gt_path = []
for folder in folders:
    gt_path.append(args.gt_path+folder)
args_training_dataset['gt_path'] = gt_path
...
with torch.no_grad():
    for epoch in range(epochs):
        gt_path = os.path.join(root_path+"_"+str(args.start_gpu*epochs + epoch), 'HR')
        lr_path = os.path.join(root_path+"_"+str(args.start_gpu*epochs + epoch), 'LR')
        os.makedirs(gt_path, exist_ok=True)
        os.makedirs(lr_path, exist_ok=True)
        for folder in os.listdir(args.gt_path):
            os.makedirs(os.path.join(gt_path, folder), exist_ok=True)
            os.makedirs(os.path.join(lr_path, folder), exist_ok=True)
        ...
        for num_batch, batch in enumerate(train_dataloader):
            ...
            lr_batch = F.interpolate(lr_batch, size=(gt_batch.size(-2), gt_batch.size(-1)), mode='bicubic',)
            ...
            for i in range(batch_size):
                ...
                lr_save_path =  os.path.join(lr_path,'{}.png'.format(batch["gt_path"][i].split("pngtxt_dir/")[-1].split(".png")[0]))
                gt_save_path =  lr_save_path.replace("/LR/", "/HR/")
                if os.path.exists(lr_save_path) and os.path.exists(gt_save_path):
                    continue
